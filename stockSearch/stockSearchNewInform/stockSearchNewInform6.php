...
var myChart = new Chart(ctx, {
  ...
  data:
  {
    labels: [
    <?php
$FundTrendData = mysql_query("SELECT date,dividend_yield FROM stock_market.Financial_report where code = '$code';");
...
?>],
    ...
    },
    {
      label: '現金股利殖利率',
      data: [<?php
$dividend_yield = mysql_query("SELECT date,cash_dividend_yield FROM stock_market.Financial_report where code = '$code';");
$tmp = 0;
for ($i = 1; $i <= mysql_num_rows($dividend_yield); $i++) {
  $rs = mysql_fetch_row($dividend_yield);
  $tmp = $tmp + $rs[1];
  ?>
<?php echo $rs[1] ?>,
<?php
