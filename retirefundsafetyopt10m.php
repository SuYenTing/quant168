...
$FundTrendData=mysql_query("select date, nav/1000000 from web_data.strategies_nav where name = 'safety.opt.10m' ");//從contact資料庫中選擇所有的資料表
...
?>],
    datasets: [{
      label: '基金走勢圖',
      data: [<?php
$FundTrendData=mysql_query("select date, nav/1000000 from web_data.strategies_nav where name = 'safety.opt.10m' ");
$tmp = 0;
for($i=1;$i<=mysql_num_rows($FundTrendData);$i++){
$rs=mysql_fetch_row($FundTrendData);
$tmp = $tmp+$rs[1];
