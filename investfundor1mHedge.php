...
$FundTrendData=mysql_query("select date, nav/1000000 from web_data.strategies_nav where name = 'or.1m' ");//從contact資料庫中選擇所有的資料表
...
<?php
$stock1=mysql_query("SELECT * FROM web_data.hedge_strategies_performance where code='or.1m';");
...
<?php
$stock=mysql_query("SELECT * FROM web_data.hedge_strategies_holding where trade_date=(select max(trade_date) from web_data.hedge_strategies_holding where name='or.1m' and type = '股票') and name='or.1m' and type = '股票';");
...
<?php
$stock=mysql_query("SELECT * FROM web_data.hedge_strategies_holding where trade_date=(select max(trade_date) from web_data.hedge_strategies_holding where name='or.1m' and type = '期貨') and name='or.1m' and type = '期貨';");
$rs=mysql_fetch_row($stock);
?>
                <tr class="content">
                    <td><?php echo $rs[4]?></td>
...
?>],
    datasets: [{
      label: '基金走勢圖',
      data: [<?php
$FundTrendData=mysql_query("select date, nav/1000000 from web_data.hedge_strategies_nav where name = 'or.1m' ");
$tmp = 0;
for($i=1;$i<=mysql_num_rows($FundTrendData);$i++){
$rs=mysql_fetch_row($FundTrendData);
$tmp = $tmp+$rs[1];
