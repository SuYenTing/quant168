...
$FundTrendData = mysql_query("select date, nav/10000000 from web_data.hedge_strategies_nav where name = 'quality.10m' "); //從contact資料庫中選擇所有的資料表
...
<?php
$stock = mysql_query("SELECT * FROM web_data.hedge_strategies_holding where YEAR(trade_date) >= (YEAR(CURDATE())-1) and name='quality.10m';");
...
<?php
$stock = mysql_query("SELECT * FROM web_data.ifiwanttobuytoday_hedge_fund where name='quality.10m';");
for ($i = 1; $i <= mysql_num_rows($stock); $i++) {
	$rs = mysql_fetch_row($stock);
	?>
                <tr class="content">
