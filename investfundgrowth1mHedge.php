...
<?php
$stock=mysql_query("SELECT * FROM web_data.hedge_strategies_holding where YEAR(trade_date) = YEAR(CURDATE()) and name='growth.1m';");
...
<?php
$stock=mysql_query("SELECT * FROM web_data.ifiwanttobuytoday_hedge_fund where name='growth.1m';");
for($i=1;$i<=mysql_num_rows($stock);$i++){
$rs=mysql_fetch_row($stock);
?>
                <tr class="content">
