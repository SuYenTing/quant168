...
<?php
$stock=mysql_query("SELECT * FROM web_data.strategies_holding where start_date=(select max(start_date) from web_data.strategies_holding where name='payout.opt.10m') and name='payout.opt.10m';");
...
?>],
    datasets: [{
      label: '基金走勢圖',
      data: [<?php
$FundTrendData=mysql_query("select date, nav/10000000 from web_data.strategies_nav where name = 'payout.opt.10m' ");
$tmp = 0;
for($i=1;$i<=mysql_num_rows($FundTrendData);$i++){
$rs=mysql_fetch_row($FundTrendData);
$tmp = $tmp+$rs[1];
