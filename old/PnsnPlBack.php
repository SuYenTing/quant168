...
<div class="container">
		...
		<table>
			...
			<tr>
				...
				<td>總共提撥</td>
				<td>
				<input type="number" id="totalGather" value=9 readonly>%</td>
			</tr>
			<tr>
				<td>投資報酬率</td>
				<td>
...
<option value="100000">$100,000</option>
					...
					<td>退休時平均餘命</td>
					<td>
					<input type="number" id="lifeLeft" value=17 readonly>歲</td>
				</tr>
			</table>
			<select id="job" onchange="jobInput()">
				<option value="">請選擇職業別</option>
				<option value="labor">勞工</option>
				<option value="gover">公務員</option>
				<option value="pubteacher">公立教職</option>
				<option value="priteacher">私立教職</option>
				<option value="soilder">軍人</option>
				<option value="farmer">農民</option>
				<option value="normal">一般民眾</option>
			</select>
			<div id="jobExtraInput"></div>
		...
		var v = w * (1 - (1 / (Math.pow((1 + roi / 12), (lifeLeft * 12))))) / (roi / 12);
		document.getElementById("w").innerHTML = w;
		document.getElementById("v").innerHTML = v;
		} else if ("farmer" == job) {
		var insuranceYear = parseInt(document.getElementById("insuranceYear").value);
		var w;
		if (insuranceYear >= 15) {
		w = 7256;
		} else {
		...
		var minus = shouldhave - accumalated; td4.appendChild(document.createTextNode("- $" + Math.round(shouldhave))); td4.style.border = '1px solid black';
		}
		accountTable.appendChild(tbl);
		}
		function jobInput() {
		if (document.getElementById("job").value == "farmer") {
		var x = document.createElement("INPUT");
		x.setAttribute("type", "text");
		x.setAttribute("id", "insuranceYear");
		//var y =document.createElement("P");
		var t = document.createTextNode("欲投保年資:");
		//y.appendChild(t);
		document.getElementById("jobExtraInput").appendChild(t);
		document.getElementById("jobExtraInput").appendChild(x);
		}
		}
